  ...
  return (
    <div>
      ...
      <div className="stats-grid" style={{marginBottom:'var(--space-5)'}}>
        ...
        <div className="glass-card stat-card emerald"><div className="stat-card-icon emerald">💰</div><div className="stat-card-value">{formatCurrency(collection.reduce((s, c) => s + parseFloat(c.totalCollected || 0), 0), true)}</div><div className="stat-card-title">Total Hall Collection</div></div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:'var(--space-5)',marginBottom:'var(--space-5)'}}>
        <div className="glass-card" style={{padding:'var(--space-6)'}}>
          ...
          <div style={{height:'350px'}}>
            <Bar data={{
              labels: top15.map(c => hallMap[c.code]?.name || c.name || c.code),
              datasets: [{ data: top15.map(c => parseFloat(c.totalCollected || 0)), backgroundColor: 'rgba(59,130,246,0.6)', borderColor: '#3b82f6', borderWidth: 1, borderRadius: 4 }]
        ...
        <div className="table-container" style={{border:'none'}}>
          <table className="data-table">
            ...
            <tbody>
              {collection.map(c => (
                <tr key={c.code}>
                  <td style={{fontWeight:500}}>{hallMap[c.code]?.name || c.name || c.code}</td>
                  <td><code style={{fontSize:'var(--text-xs)',background:'var(--bg-hover)',padding:'2px 6px',borderRadius:'4px'}}>{c.code}</code></td>
                  <td><span style={{padding:'2px 8px',borderRadius:'var(--radius-full)',fontSize:'var(--text-xs)',fontWeight:600, background: (hallMap[c.code]?.gender || c.gender) === 'Boys' ? 'var(--accent-blue-dim)' : (hallMap[c.code]?.gender || c.gender) === 'Girls' ? 'var(--accent-rose-dim)' : 'var(--bg-hover)', color: (hallMap[c.code]?.gender || c.gender) === 'Boys' ? 'var(--accent-blue-soft)' : (hallMap[c.code]?.gender || c.gender) === 'Girls' ? 'var(--accent-rose-soft)' : 'var(--text-secondary)'}}>{hallMap[c.code]?.gender || c.gender || '—'}</span></td>
                  <td>{parseInt(c.studentCount || 0).toLocaleString('en-IN')}</td>
                  <td style={{fontWeight:600,color:'var(--accent-emerald-soft)'}}>{formatCurrency(c.totalCollected)}</td>
                </tr>
              ))}
            </tbody>
          </table>
